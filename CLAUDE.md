...
| `.claude/hooks/*` | 인터셉터 | 도구 호출 전후를 가로채 팀 컨벤션대로 행동을 교정. 현재 5개 작성됨: `block-main-commit.sh`(main 브랜치 직접 커밋 차단), `pre-push-check.sh`(git push 전 check-all.sh 게이트), `post-edit-format.sh`(Java 파일 편집 후 Spotless 자동 포맷), `check-writing-style.sh`(응답 종료 시 글쓰기 스타일 검사), `stop-eof-newline.sh`(세션 종료 시 EOF 개행 자동 보정) |
