...
- **`gh pr create` 실행 전 반드시 `code-reviewer` 에이전트를 호출한다.** 빌드/테스트/이슈 TODO/커밋 트레일러처럼 결정론적인 항목은 훅이 강제하지만, 계층 의존 방향·컨벤션 위반처럼 판단이 섞이는 항목은 이 스킬 절차에 박아둔 `code-reviewer` 호출이 유일한 검토 지점이다(정적분석 도구 미도입 사유는 `architecture` 스킬 참고). CRITICAL/MAJOR 발견 사항은 PR 생성 전에 수정하고, MINOR/INFO는 판단해 반영하거나 근거를 남기고 넘어간다.

## 절차

1. **`code-reviewer` 에이전트를 호출해 이번 브랜치의 변경 사항을 리뷰한다.** CRITICAL/MAJOR 발견 사항이 있으면 먼저 고치고 다시 리뷰한 뒤 다음 단계로 넘어간다.
2. PR 유형 체크박스에서 해당하는 항목 전부에 체크한다. 새로운 기능 추가, 리팩토링, 버그 수정, 설정, 문서화, 테스트 코드 추가, 기타 변경 사항 중 여러 개가 동시에 해당할 수 있다.
3. 작업 내용을 작성한다. 기능·모듈 단위로 이모지 소제목을 나누고, 그 아래 구체적인 클래스·메서드 이름과 함께 변경 사항을 나열한다.
4. 관련 이슈를 `closes #이슈번호`로 명시한다.
5. 필요하면 추가 사항에 리뷰어가 알아야 할 맥락을 적는다.
6. 제목의 대표 `Type`에 대응하는 라벨을 정한다 (예: `Docs` → `📄 docs`).
7. 생성한다. `--assignee @me`와 6번에서 정한 `--label`을 빠뜨리지 않는다.
   ```bash
