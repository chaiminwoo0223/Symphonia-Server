...
#
# 배경(이슈 #25): 세션의 어트리뷰션 기본값이 Co-Authored-By/Claude-Session 트레일러를 붙이라고 지시해도 이 저장소는 그 트레일러를 원하지 않는다(commit-push 스킬 참고).
# 사람 기억에 의존하지 않고 훅으로 결정론적으로 막는다.
# heredoc(-m/--body "$(cat <<'EOF' ... EOF)")으로 작성된 메시지도 command 문자열 안에 그대로 들어있으므로 실행 전 raw command 텍스트를 그대로 검사한다.
# -m 없이 에디터로 메시지를 작성하는 커밋(예: --amend --no-edit)은 최종 메시지를 알 수 없으므로 검증하지 않고 통과시킨다(fail-open).
# git 트레일러는 항상 줄 시작에 `Key: Value` 형태로만 온다(RFC 822 스타일). 이 규칙 자체를 설명하는
# 문서/PR 본문 문장("훅이 Co-Authored-By: Claude ...를 차단한다" 같은 프로즈)까지 오탐하지 않도록
# 패턴을 줄 시작(선행 공백만 허용)에 앵커한다.
...
if grep -qiE '^[[:space:]]*(co-authored-by:[[:space:]]*claude|claude-session:)' <<<"$command"; then
